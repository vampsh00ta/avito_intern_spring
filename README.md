...
## 1.Так как фича и тэг по условию  - уникальное значение, было решено сделать индекс (tag_id,feature_id). Из-за этого скорость поиска повышается,однако  скорость изменения уменьшается.Также реализовано кэширование через  Redis на поиск баннера для  пользователя  с временем жизни 5 минут. С флагом use_last_revision пользователь получает некэшированные данные  
## 2.Нагрузочное тестирование не проведено
## 3.История изменений реализована. Так как по условию нужно смотреть последние 3 значения, у меня возникла, как  это реализовать. Я решил создать отдельную таблицу с историей и метод, который с заданным интервалом оставляет только  3 последних изменения. Остальные удаляются
