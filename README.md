...
1. Так как фича и тэг по условию  - уникальное значение, было решено сделать индекс (feature_id,tag_id). Из-за этого скорость поиска повышается, однако  скорость изменения уменьшается. Также реализовано кэширование   Redis для метода  /banner_user   с временем жизни кэша 5 минут. С флагом use_last_revision пользователь получает некэшированные данные напрямую из PostgresQL  
2. Нагрузочное тестирование не проведено
3. История изменений реализована. Так как по условию нужно смотреть последние 3 значения, у меня возникла дилемма, как  это реализовать. Я решил создать отдельную таблицу с историей и метод, который с заданным интервалом оставляет только  3 последних изменения. Остальные записи удаляются
